...
def imprimeDEF():
        ...
        if daadReady:
                ...
                print('#endif', file=f)
                print('', file=f)
                print('#ifdef "splitModeON"', file=f)
                if english:
...
def imprimeVOC2():
        ...
        print('SV      26      verb', file=f)
        print('LOAD    27      verb', file=f)
        print('LD      27      verb', file=f)
...
def imprimePRO0_DR():
        ...
        print('                SET fDarkF', file=f)
        print('', file=f)
        # 1.0.4b6 Lo que sigue es un copia-pega del PRO 0 de DAAD-Ready 0.9.2 ya que no hay tiempo de hacer una versión específica para Triz2DAAD. Si todo va bien, funcionará.
        if english:
            print('; What follows is essentially a copy-paste from DAAD Ready 0.9.2 PRO 0 template.', file = f)
            print('; Hopefully, it will work!', file = f)
        else:
            print('; Lo que sigue es básicamente un copia-pega de la plantilla del PRO 0 de DAAD Ready 0.9.2', file = f)
        ...
        print('              SKIP $pictureOK', file = f)
        print('', file=f)
        print('#else', file=f)
        print('    #ifndef "tape48"', file = f)
        print('    >', file=f)
        print('    _       _     ZERO fDarkF', file = f)
        print('                  CLEAR 20', file = f)
        print('                  XPICTURE @fPlayer', file = f)
        print('                  ZERO 20', file=f)
        print('                  SKIP $pictureOK', file = f)
        print('    #endif', file=f)
        print('#endif', file=f)
        print('', file = f)
        print('#ifndef "splitModeON"', file = f)
        print('>', file = f)
        print('_       _     WINDOW 1', file = f)
        ...
        print('                  XNEXTCLS', file = f)
        print('              #endif', file = f)
        print('#else', file = f)
        print('>', file = f)
        print('_       _     CLS', file = f)
        print('#endif', file = f)
        print('', file = f)
        print('$pictureOK', file = f)
        print('', file = f)
        print('>', file = f)
        print('_       _     WINDOW 1', file = f)
        print('              NOTZERO fDarkF', file = f)
...
def imprimePRO5(helpMessage1, helpMessage2, listRooms2):
        ...
        if dsf:
                print('>', file=f)
        if daadReady:
                print('SAVE    _       #ifndef "tape48"', file=f)
        ...
        if dsf:
                print('>', file=f)
        if daadReady:
                print('LOAD    _       #ifndef "tape48"', file=f)
...
def imprimePRO6_DR():
        ...
        print('', file=f)
        if english:
            print('; What follows is essentially a copy-paste from DAAD-Ready 0.9.2 routine.', file = f)
            print('; Hopefully, it will work!', file=f)
        else:
            print('; Lo que sigue es básicamente un copia-pega de la rutina de DAAD-Ready 0.9.2', file = f)
            print('; ¡Con suerte, funcionará!', file=f)
        print('', file = f)
        print('>', file = f)
        print('_       _     #ifdef "pc"', file = f)
        ...
        print('              WINDOW  1', file = f)
        print('              WINAT 14 0', file = f)
        print('              WINSIZE 25 128', file = f)
        print('              WINDOW   0', file = f)
        print('              CLS', file = f)
        print('              #ifdef "nativeraster"', file = f)
        print('                  PICTURE 0', file = f)
        print('                  DISPLAY 0', file = f)
        print('                  SKIP $picture0OK', file = f)
        print('              #else', file = f)
        print('                  #ifndef "tape48"', file = f)
        print('                      CLEAR 20', file = f)
...
for y in listRooms:
    if english:
        if y['name'].startswith('Location ') == False:
            if y['description'].startswith('Location '):
                y['description'] += ': ' + y['name']
    else:
        if y['name'].startswith('Localidad ') == False:
            if y['description'].startswith('Descripción de localidad '):
                y['description'] += ': ' + y['name']
